...
def new_Array(cantidad_elementos: int) -> list:
    """Crea un arreglo inicializado con 0 con la cantidad de elementos
    indicada en parametro

    Args:
        cantidad_elementos (int): cantidad de elementos del arreglo

    Returns:
        list: array de enteros con cantidad de elementos ingresados por
        parametro
    """
...
def crear_array(mensaje: str, mensaje_error: str, minimo: int,
                maximo: int, reintentos: int,
                cantidad_elementos: int) -> list | None:
    
    """Crea un arreglo y permite el ingreso por consola de los elementos

    Args:
        mensaje (str): Cadena de caracteres que se muestra para el ingreso de
        elementos
        mensaje_error (str): Cadena de caracteres que muestra error si no se
        puede validar
