...
def get_float(mensaje: str, mensaje_error: str, minimo: int, maximo: int, reintentos: int) -> float | None:
    ...
    Returns:
        float | None: regresa un numero flotante ya validado o None si no hay un valor valido
    """
...
def get_string(mensaje: str, mensaje_error: str,  reintentos: int, longitud: int) -> str | None:
    ...
    Args:
        ...
        mensaje_error (str): un string de error de ingreso, reintente
        reintentos (int): entero que representa cantidad de veces que se aceptan los reingresos
        longitud (int): es un entero que representa la cantidad de caracteres permitidos
...
numeroAMostrar = get_int("Ingrese numero: ", "Numero fuera de rango, reingrese: ", 0, 10, 3)

print(numeroAMostrar)